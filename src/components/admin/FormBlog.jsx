...
return false;
  ...
  const handleImageChange = (e) => {
    setBlogImage(e.target.files[0]); // Save the file
  };
  ...
  const saveAdded = async () => {
    ...
    console.log(blogImage, "image fileee")
    const result = handleValidation();
    ...
    <div className="p-8">
      ...
      <div className="flex gap-5 float-right">
        ...
        <div
          ...
          className={`${
            isSaveClicked ? "bg-white text-black" : "bg-[#000000] text-white"
          } cursor-pointer px-6 float-right mb-14 py-3 border-none rounded-full hover:bg-white hover:text-black transition-all duration-500 shadow-[#0000005f] shadow-md  text-sm text-center w-fit mt-14 font-bold`}
        >
          SAVE
        </div>
        {isSaveClicked ? (
